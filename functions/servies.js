...
const getAllServices = async (req, res) => {
    try {
        const getQuery = 'select * from serviceorders';
...
const getServiceOrderById = async (req, res) => {
    const { id } = req.params
    try {
        const query = `select * from serviceorders where id= '${id}'`;
...
const deleteServiceOrders = async (req, res) => {
    const { id } = req.body
    try {
        const deleteQuery = `DELETE FROM serviceorders WHERE id = '${id}'`;
...
const createServiceOrders = async (req, res) => {
    const { id, name } = req.body;
    try {
        const date = new Date();
        const insertQuery = `INSERT INTO serviceorders (id, name)
        VALUES ('${id}','${name}')
        RETURNING id;`
        const result = await connection.query(insertQuery);
        // Check if the insertion was successful
...
export { getAllServices, getServiceOrderById, deleteServiceOrders, createServiceOrders }
