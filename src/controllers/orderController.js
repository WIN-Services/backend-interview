...
const createOrder = async (req, res) => {
  try {
    ...
    if (!validator.isValidRequestBody(req.body)) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter Valid order Details" });
    }
    if (!validator.isValid(totalfee)) {
      return res
        .status(400)
        .send({ status: false, message: "total fee missing" });
    }
    if (!validator.isValid(services)) {
      return res
        .status(400)
        .send({ status: false, message: "service field missing" });
    }
    if (!validator.isValidObjectId(services)) {
      return res
        .status(400)
        .send({ status: false, message: "invalid service" });
    }
    ...
    if (!findService) {
      return res
        .status(404)
        .send({ status: false, message: "no such service found" });
    }

    let checkExistOrder = await orderModel.find({
      $and: [
    ...
    if (checkExistOrder && checkExistOrder.length > 0) {
      return res
        .status(409)
        .send({ status: false, message: "order already exist" });
    }
  ...
  try {
    ...
    if (req.params.orderId) {
      ...
      if (!findOrder) {
        return res
          .status(404)
          .send({ status: false, message: "no such order found" });
      }
...
const updateOrder = async (req, res) => {
  try {
    ...
    let order = await orderModel.findByIdAndUpdate(orderId, payload, {
      new: true,
    });
